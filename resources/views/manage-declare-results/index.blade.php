    ...
    <div class="col-md-10">
        <form class="card elevation-2 col-12" action={{route('declares.store')}} method="POST">
            ...
            <div class="form-group">
                ...
                <select name="set_id" id="set_id" class="form-control col-12 @error('set_id') is-invalid @enderror">
                    <option value="">select set</option>

                    @foreach ($sets as $set)
                        <option value="{{$set->id}}" @if(old('set_id')==$set->id) selected @endif>{{$set->set_name}}</option>
                    @endforeach
                </select>
                @error('set_id')
                    <span class="invalid-feedback text-danger">{{$message}}</span>
