...
EchartMain.registerTheme('ovilia-green', theme)
    ...
    export default {
        ...
        computed:{
            ...mapState({
                payments:state=>state.expensesModule.payments,
                paymentPagination:state=>state.expensesModule.paymentPagination.page,
                totalpayments:state=>state.expensesModule.totalpayments,
                queryType:state=>state.expensesModule.queryType,
            }),
            getData(){
                let dataBar = {};
                if(this.queryType === 'daily'){
                    ...
                    this.payments.forEach(item=>{
                        xAxisData.push(item.expense_day);
                ...
                else if (this.queryType === 'daily_by_group')
                {
                    ...
                    this.payments.forEach(item=>{
                        xAxisData.push(item.expense_day);
                        data1.push((item.expensetype).toLowerCase() === '_others'?item.total_amount:0);
                        data2.push((item.expensetype).toLowerCase() === 'salary'?item.total_amount:0);
                        data3.push((item.expensetype).toLowerCase() === 'welfare'?item.total_amount:0);
                    });

                    dataBar = {
                        title:{
                            text:"Daily Expenses by Groups",
                            left:'center',
                            top:20
                        },
                ...
                else if(this.queryType === 'weekly_by_group')
                {
                    ...
                    dataBar = {
                        title:{
                            text:"Weekly Expenses by Groups",
                            left:'center',
                            top:20
                        },
                    ...
                    this.payments.forEach(item=>{
                        xAxisData.push(item.month_of_the_year+"/"+item.year);
                        data.push(item.total_amount);
                    });
                    ...
                    dataBar = {
                        ...
                        title:{
                            text:"Monthly Expenses by Groups",
                            left:'center',
                            top:20
                        },
                        legend:{
                    ...
                    dataBar = {
                        title:{
                            text:"Yearly Expenses by Groups",
                            left:'center',
                            top:20
                        },
                        grid:{
        ...
        methods:{
            ...
            getPayments(){
                ...
                this.$store.dispatch('expensesModule/GET_EXPENSES_INCOME_ACTION',data).finally(()=>{
