<template>
  <v-sheet class="row col white p-2">
      ...
      <table class="table table-sm col-12" style="font-size:12px;">
          ...
          <tbody>
              <tr v-if="makeBorrowItems.length === 0">
                  <td colspan="5">
                      <p>No Account Borrowed From yet</p>
                  </td>
              </tr>
              <tr v-for="item in makeBorrowItems" :key="item.selectedAccount.id" :class="(item.selectedAccount.balance - item.credit)<0?'bg-danger text-white':''">
                  <td>
                  ...
                  <td>
                      ...
                      v-on:click="deleteBorrowItem(item)"
                      >
                          Remove
                      </button>
                  </td>
          ...
          <tfoot>
              <tr>
                  ...
                  <td>Remaining</td>
                  <td :class="gettingRemaining<0?'bg-warning text-white':((gettingRemaining)===0?'bg-success text-white':'')">{{ (gettingRemaining) | currency }}</td>
              </tr>
              <tr>
...
export default {
    ...
    computed:{
        ...mapState({
            ...
            makeBorrowItems:state=>state.makeBorrowItems,
            totalSubstraction : state=>state.amntTtlExpBorSub,
        }),
        gettingSum(){
            let totalBorrow = 0;
            this.makeBorrowItems.forEach(element=>{
                totalBorrow += parseInt(element.credit);
            });
            this.$store.dispatch("getCreditTotalAction",totalBorrow);
    ...
    methods:{
        ...mapActions({
         deleteBorrowItem:"deleteBorrowItemAction"
     }),
     classState(){
         let className = "";
