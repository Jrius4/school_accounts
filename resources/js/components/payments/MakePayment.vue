      ...
      <v-container fluid>
            ...
            >
              <template v-slot:selection="data">
                ...
                >
                  <v-avatar left>
                    <v-img
                      :src="`/images/${data.item.image!== null?data.item.image:'user_all.png'}`"
                    ></v-img>
                  </v-avatar>
                ...
                <v-row align="center" justify="center">
                  <v-col cols="12" sm="3">
                    <img
                      ...
                      height="35"
                      class="img-circle"
                      :src="`/images/${item.image!== null?item.image:'user_all.png'}`"
                    />
                  </v-col>
                  <!-- <v-spacer></v-spacer> -->
                  <v-col cols="12" sm="6" class="mx-1">
                    <h6
                      v-html="`${item.name} ${item.schclass_id?"/ "+item.schclass.name:""} ${item.roll_number}`"
                    />
                  </v-col>
                </v-row>
              </template>
          ...
          <v-col cols="12" xl="7" lg="7" md="7" v-if="student !== null">
            <table class="table table-sm">
              ...
              <tbody>
                ...
                <tr>
                  <th>School Fees</th>
                  <th>{{`${studentSelection.fees_to_be_paid}` | currency}}</th>
                </tr>
                <tr>
                  <th>Balance</th>
                  <th>{{`${studentSelection.fees_to_be_paid - studentSelection.amount_paid}` | currency}}</th>
                </tr>
              </tbody>
            </table>
          </v-col>
          <v-col cols="12" xl="7" lg="7" md="7" v-if="student === null">
            <v-text-field v-model="studentName" label="Student Name"></v-text-field>
            <v-text-field v-model="studentClass" label="Student Class"></v-text-field>
          </v-col>
        </v-row>
        <v-row flat justify="center" class="p-2 teal darken-3">
          <v-col cols="12" md="6">
            ...
            <v-sheet>
              <base-material-card icon="mdi-bank" title="Accounts" class="elevation-4">
                <v-window v-model="accStep">
                  <v-window-item :value="1">
                    <v-data-table
                      dense
                      :search="searchAcc"
                      :item-key="accountSortRowsBy"
                      :headers="headersAcc"
                      :items="accounts"
                      :sort-by="accountSortRowsBy"
                      class="mr-2"
                      :loading="loadingAcc"
                  ...
                  <v-window-item :value="2">
                      ...
                      <v-col cols="12">
                        <table class="table table-sm">
                          ...
                          <tbody v-if="accountSelected!==null">
                            <tr
                              v-for="(fee,ind) in accountSelected.fee_structures"
                              :key="`acc-${ind}`"
                            >
                    ...
                    <v-row flat>
                      <v-col cols="12">
                        <v-text-field v-model="amountDepositFormat" label="Amount Deposited"></v-text-field>
                        <v-text-field v-model="amountBalanceFormat" label="Amount Balance"></v-text-field>
                      </v-col>
                      <v-col cols="12">
                        <v-btn
                          small
                  ...
                  <table class="table table-sm" style="font-size:0.8rem">
                    ...
                    <tbody>
                      ...
                      <tr v-if="activePayments.length>0">
                        <td>Active Total</td>
                        <td>{{this.fullAmount | currency}}</td>
                        <td>Active Balance</td>
                        <td>{{this.activeBalance | currency}}</td>
                      </tr>
                      <tr v-if="activePayments.length === 0">
        ...
        <v-row flat>
          ...
          <v-col cols="12" sm="6">
            <v-text-field v-model="paid_by" label="Paid By"></v-text-field>
          </v-col>
        </v-row>
        <v-row flat>
          <v-col cols="12" sm="6">
            <v-text-field v-model="activeBalanceFormat" disabled label="Computed Balance"></v-text-field>
            <v-text-field v-model="balanceFormat" label="Amount Balance"></v-text-field>
          </v-col>
          <v-col cols="12" sm="6">
            <v-textarea cols="6" rows="3" v-model="description" label="Description"></v-textarea>
          </v-col>
        </v-row>
        <v-row flat>
          <v-col cols="12">
            <v-btn small dark color="teal darken-3" class="btn-block" @click="makePayment">Save</v-btn>
            <v-btn
              small
              dark
              color="grey darken-3"
              class="btn-block"
              @click="cancelMakePayment"
            >Cancel</v-btn>
          </v-col>
        </v-row>
      </v-container>
  ...
  methods: {
    getAccounts() {
      ...
      return new Promise((resolve, reject) => {
        ...
        if (!search) {
          ...
          this.$store
            .dispatch("GET_ACCOUNTS_ACTION", pagination)
            .finally(() => {
    ...
    loadPayment() {
      let index = this.activePayments.findIndex(
        (c) => c.account_name === this.accountSelected.account_name
      );
      if (index == -1) {
    ...
    makePayment() {
      ...
      this.$store
        .dispatch("makePaymentsModule/MAKE_PAYMENTS_ACTION", formData)
        .finally(() => {
          this.term_id = null;
