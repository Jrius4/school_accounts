...
export default new Vuex.Store({
    ...
    mutations: {
        ...
        saveExpenseItem(currentState, item) {
            let index = currentState.expenseItems.findIndex(
                c => c.id == item.id
            );
            if (index == -1) {
        ...
        makeBorrow(currentState, borrow) {
            let index = currentState.makeBorrowItems.findIndex(
                c => c.selectedAccount.id == borrow.selectedAccount.id
            );
            if (index == -1) {
        ...
        makeBorrowLoan(currentState, borrow) {
            let index = currentState.makeLoanBorrowItems.findIndex(
                c => c.selectedAccount.id == borrow.selectedAccount.id
            );
            if (index == -1) {
        ...
        deleteBorrowItem(currentState, data) {
            let index = currentState.makeBorrowItems.findIndex(
                c => c.selectedAccount.id == data.selectedAccount.id
            );
        ...
        SET_EMPLOYEES(currentState, payload) {
            ...
            currentState.employeePagination.page = parseInt(
                payload.current_page
            );
            currentState.employeePagination.rowsPerPage = parseInt(
                payload.per_page
            );
        ...
        GET_STUDENTS(currentState, payload) {
            ...
            currentState.studentPagination.page = parseInt(
                payload.current_page
            );
            currentState.studentPagination.rowsPerPage = parseInt(
                payload.per_page
            );
        ...
        GET_EXPENSES_FULL(currentState, payload) {
            ...
            currentState.expensePagination.page = parseInt(
                payload.current_page
            );
            currentState.expensePagination.rowsPerPage = parseInt(
                payload.per_page
            );
    ...
    getters: {
        ...
        orderedCities(state) {
            return state.cities
                .concat()
                .sort((c1, c2) => c2.sqmiles - c1.sqmiles);
        },
        filteredCities(state, getters) {
            // return (amount) => getters.orderedCities.filter(c=>c.sqmiles > amount);
            return str_search =>
                getters.orderedCities.filter(c =>
                    c.country.includes(str_search)
                );
        },
        itemFilteredByCategory: state =>
            state.items.filter(
                item =>
        ...
        processedItems: (state, getters) => {
            ...
            return getters.itemFilteredByCategory.slice(
                index,
    ...
    actions: {
        ...
        async saveCityAction(context, city) {
            ...
            if (index == -1) {
                ...
                city = (await Axios.put(`${citiesUrl}/${city.id}`, city)).data
                    .data;
            }
        ...
        async createExpenseItemAction(context, item) {
            item = (
                await Axios.post("/api/make-expenses", item, {
                    headers: {
                        Authorization: "Bearer " + context.state.token
                    }
                })
        ...
        async updateExpenseItemAction(context, item) {
            item = (
                await Axios.put(`/api/make-expenses/${item.id}`, item, {
                    headers: {
                        Authorization: "Bearer " + context.state.token
                    }
                })
        ...
        async deleteExpItemAction(context, item) {
            let total = (await Axios.delete(`/api/make-expenses/${item.id}`))
                .data.totalItems;
            context.commit("getExpenseTotal", total);
        ...
        async searchEmployeesAction(context, query) {
            ...
            Axios.get("/api/v1/getEmployees", { params })
                .then(res => {
        ...
        async getEmployeesPaginationAction(context, sortBy) {
            if (sortBy.val !== null)
                return new Promise((resolve, reject) => {
                    let page_number =
                        sortBy.page || context.state.employeePagination.page;
                    let rowsPerPage =
                        sortBy.rowsPerPage ||
                        context.state.employeePagination.rowsPerPage;
                    let sortRowsBy =
                        sortBy.sortRowsBy || context.state.EmployeeSortRowsBy;
                    let sortDesc = sortBy.sortDesc || false;

                    Axios.get(
                        "/api/v1/getEmployees?page=" +
                            page_number +
                            "&rowsPerPage=" +
                            rowsPerPage,
                        {
