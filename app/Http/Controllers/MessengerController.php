...
class MessengerController extends Controller
{
    public function index()
    {
        ...
        $users = DB::select("SELECT A.id,A.last_name,A.first_name,A.email,A.image,count(is_read) as unread FROM users A LEFT JOIN messages B ON A.id = B.from AND B.to =" . auth()->user()->id . " WHERE A.id != " . auth()->user()->id . " GROUP BY A.id,A.last_name,A.first_name,A.email,A.image ");
