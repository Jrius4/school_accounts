...
$sql2="SELECT id FROM tblresultmt WHERE SubjectId=$s AND StudentRoll='$sRoll' AND Year=$years  AND Term=1;";
  ...
  $pid=$pid1[$i];
		...
		if($count1<=0){
			header("Location: ../teachert1mt.php?marks=ttpernotset");
        exit();
		}else{
		$final=($mar/100)*$per;
		$sql4="INSERT INTO tblfinalmt(SubjectId,PaperId,StudentRoll,Term,FinalMark,Year) VALUES($s,$pid,'$sRoll',1,$final,$years)";
		mysqli_query($conn,$sql4);
		
		$sql1="INSERT INTO  tblresultmt(StudentRoll,ClassId,SubjectId,PaperId,Marks,Term,Year) VALUES('$sRoll',$cid,$s,$pid,$mar,1,$years);";
		mysqli_query($conn,$sql1);
	}
	}
	header("Location: ../teachert1mt.php?marks=success");
        exit();
	}
}else{
	 header("Location: ../teachert1mt.php?marks=notset");
        exit();
}